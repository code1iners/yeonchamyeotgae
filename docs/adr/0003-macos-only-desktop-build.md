...
- 기존 GitHub Actions의 Windows 매트릭스와 Windows `.exe` 패키징 정의는 새 지원 범위와 맞지
  않는 잔여 설정이다. 이번 결정 기록에서는 자동화를 변경하지 않으며, 워크플로를 다음에 수정할
  때 Windows 잡을 복구하지 않고 제거한다.
- 현재 원격 Windows 잡의 성공 여부는 제품 수용이나 릴리스 준비의 증거로 사용하지 않는다.
