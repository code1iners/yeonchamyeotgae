...
현재 앱 버전과 같은 태그가 이미 있고 그 태그의 공개 Release만 없는 경우에는 로컬·원격 태그의
peeled commit(주석 태그가 실제로 가리키는 커밋)을 확인한 뒤 같은 태그를 재사용합니다. 이때
태그를 다시 push하지 않고 해당 태그의 Release 워크플로만 확인하므로, 실패한 실행은 출력된
`gh run rerun <실행 ID>`로 복구합니다. 로컬·원격 태그가 서로 다른 커밋을 가리키거나 현재
버전과 관계없는 미게시 태그가 있으면 안전을 위해 중단합니다.
