...
긴 발생분·휴가 이력 목록만 별도 스크롤한다. 요약의 검산 행과 `휴가 등록`은 목록을 줄여도
첫 뷰포트에 남는다.
...
읽게 하는 고정 기준이다. 긴 발생분 목록은 최대 240px, 이력 리스트는 최대 356px 높이에서
각각 스크롤하고, 나머지 화면은 내용에 따라 팝오버 높이가 변한다. 별도의 모바일 브레이크포인트나
반응형 재배치는 현재 시스템에 없다.
...
- **Saving:** 저장 중에는 등록면에 `저장 중입니다…`를 live status로 표시하고 모든 입력과
  ...
  셸의 상태 푸시가 잔여·요약·이력·트레이를 함께 갱신한 뒤 등록면을 닫는다.
...
- **Confirmation:** 가져오기는 파일 선택 전에 `지금 데이터가 대체됩니다`와 전체 교체,
  `data.json.bak` 백업 사실을 확인 영역에서 읽힌다. 취소하면 현재 원장·설정 입력·기존 안내를
  그대로 두고 가져오기 버튼으로 포커스를 돌린다.
...
- **Grant Rows:** 살아 있는 발생분의 `남은 양/총량` 수량 열은 68px, 소멸일 또는 D-day 열은 84px로
  고정해 값의 자릿수와 상태 배지가 바뀌어도 같은 자리에 선다.
