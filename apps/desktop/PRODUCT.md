...
앱은 macOS 메뉴 막대에 잔여를 숫자로 상시 표시하며, 사용자가 앱을
...
- 앱은 상주하며 macOS 메뉴 막대에서 동작하고 Dock을
  차지하지 않는다.
...
- 계산 엔진은 플랫폼과 입출력을 모르는 순수 TypeScript 패키지이고, 셸은 Electron·React로
  ...
  한다.
- 배포 대상은 macOS Apple Silicon용 DMG이며 GitHub Release에서 제공한다. 첫 버전은
  코드 서명·공증과 자동 업데이트를 제공하지 않고 수동 설치·업데이트를
  안내한다.
