...
- [x] 실제 CI가 만든 새 릴리스 DMG로 다시 검증한다 — v0.1.3 릴리스 DMG로 재검증 완료
      (아래 "추가 검증" 참고). Finder 더블클릭 다이얼로그 UI 자체는 여전히 미확인 —
      "남은 위험" 참고.
...
**추가 검증(v0.1.3 — 실제 CI가 만든 릴리스 DMG)**: 위 로컬 검증은 이 머신에서 다시 패키징한
`.app`을 대상으로 했다 — 실제 CI 산출물이 아니었다는 점에서 아래 acceptance criterion과
어긋나 있었다. 이후 GitHub Actions가 태그 `v0.1.3`에서 실제로 만들어 릴리스에 올린
`yeonchamyeotgae-0.1.3-arm64.dmg`를 받아 재검증했다:

1. DMG 자체에 `com.apple.quarantine`을 부여해 마운트, `.app`을 별도 폴더로 복사한 뒤
   같은 격리 속성을 다시 부여해 실제 다운로드 조건을 흉내냈다.
2. `codesign --verify --deep --strict "연차몇개.app"` → 통과(종료 코드 0).
3. `spctl -a -vvv --type execute "연차몇개.app"` → `rejected`(종료 코드 3) — "damaged"가
   아니라 README/RELEASE_BODY.md가 안내하는 "확인되지 않은 개발자" 흐름과 일치.

**남은 위험**: 이 검증은 `spctl`의 정적 평가까지만 확인했고, 실제 Finder 더블클릭 →
"확인되지 않은 개발자" 다이얼로그 → 시스템 설정 "그래도 열기" 전체 UI 흐름은 이 실행
환경(GUI 세션 제약)에서 끝까지 재현하지 못했다. `spctl`의 판정이 Finder가 보여주는 실제
다이얼로그의 신뢰 가능한 대리 지표이긴 하지만, 100% 동일하다는 보장은 아니다. 사용자가
직접 더블클릭해 다이얼로그 흐름을 눈으로 확인하는 것이 남은 최종 확인이다.
